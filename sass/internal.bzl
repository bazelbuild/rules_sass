...
"""Internal Sass implementation.

This is separate from sass.bzl to make it easier to change the API between
open-source usage and Google-internal use."""

ALLOWED_SRC_FILE_EXTENSIONS = [".sass", ".scss", ".css", ".svg", ".png", ".gif", ".cur", ".jpg", ".webp"]

# Documentation for switching which compiler is used
COMPILER_ATTR_DOC = """Choose which Sass compiler binary to use.
...
def sass_binary_outputs(src, output_name, output_dir, sourcemap):
    ...
    output_name = output_name or strip_extension(src.name) + ".css"
...
_sass_deps_attr = attr.label_list(
